			...
			<div class="section-content" :class="{empty:!imgs.length}" etext="待上传图片——无">
				<div class="upload-list">
					<div class="album-card" v-for="item in imgs">
						<a class="picture" :style="item.img|bgUrl" :class="{uploading:item.uploading}"><a @click.stop="delImg(item)" class="iconfont icon-trash"></a></a>
						<a class="title">{{item.des}}</a>
					</div>
				</div>
			...
			add2wait(){
        		if (this.tmpImg&&this.tmpDes){
        			if (this.imgs.length<5){
						this.imgs.push({type:this.tmpType,des:this.tmpDes,img:this.tmpImg,uploading:false});
						this.tmpImg = null;
						this.tmpDes = '';
